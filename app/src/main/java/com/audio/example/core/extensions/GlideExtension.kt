...
fun loadImageGlide(viewGroup: ViewGroup, path: String, imageView: ImageView, isLoadShimmer: Boolean = true, imageError: Int) {
    val shimmerDrawable = ShimmerDrawable().apply {
        setShimmer(DataLocal.shimmer)
    }
    if (isLoadShimmer){
        Glide.with(viewGroup).load(path).placeholder(shimmerDrawable).error(imageError).into(imageView)
    }else{
        Glide.with(viewGroup).load(path).placeholder(shimmerDrawable).error(imageError).into(imageView)
    }
}
